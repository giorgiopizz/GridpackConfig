...
make_gridpack () {
    ...
    #           echo "set cluster_temp_path `echo $RUNHOME`" >> mgconfigscript 
      ...
      # cd ../.. 
      if [ -e $CARDSDIR/restrict*.dat ]; then

        echo "Loading restrict card in SMEFTsim"
        ls $CARDSDIR/restrict*.dat
        cp $CARDSDIR/restrict*.dat models/$SMEFTBASEDIRORIG/
        ls models/$SMEFTBASEDIRORIG/
      fi
